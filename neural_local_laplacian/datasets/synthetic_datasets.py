...
class SyntheticSurfaceDataset(ABC, Dataset):
    ...
    def _generate_surfaces(self) -> List[Data]:
        ...
        # generated by that call (matching how grid_radius is shared). The
        # per-point Gaussian noise itself is still drawn fresh per surface
        # from torch's global RNG (so the same idx can see different noise
        # realizations across epochs, while the std is deterministic in idx).
        lo, hi = self._position_noise_std_range
        if hi > 0:
            noise_std = float(self._rng.uniform(lo, hi)) if hi > lo else hi
            if noise_std > 0:
                for surface in surfaces:
                    noise = torch.randn_like(surface.pos) * noise_std
