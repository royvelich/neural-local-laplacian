...
def _worker(rank: int, world_size: int, cfg_dict: dict, output_dir: str, total_meshes: int):
    ...
    # Bypasses MeshDataset.get() — reads file paths from dataset,
    # then loads meshes + geodesic caches directly.
    # ==================================================================
    ...
    print(f"{tag}Loading done: {t_load_elapsed:.1f}s total  "
          f"(load_mesh={t_loadmesh_total:.1f}s  geodata={t_geodata_total:.1f}s)",
          flush=True)
