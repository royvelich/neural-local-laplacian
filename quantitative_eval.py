...
from neural_local_laplacian.utils.laplacian_assembly import LaplacianConfig
...
def _worker(rank: int, world_size: int, cfg_dict: dict, output_dir: str, total_meshes: int):
    ...
    top_k_configs = [
        LaplacianConfig(assembly=pred_lap_config.assembly, pruning='topk', k_prune=tk)
    ...
    for local_idx, global_idx in enumerate(my_indices):
        try:
            ...
            # Reuse vertices/faces from dataset (avoids double load_mesh call)
            vertices = data.raw_vertices.numpy() if hasattr(data.raw_vertices, 'numpy') else np.array(data.raw_vertices)
            faces = data.raw_faces.numpy() if hasattr(data.raw_faces, 'numpy') else np.array(data.raw_faces)
            N = len(vertices)
    ...
    print(f"{tag}Loading done: {t_load_elapsed:.1f}s total  "
          f"(dataset={t_dataset_total:.1f}s  geodata={t_geodata_total:.1f}s)",
          flush=True)
        ...
        for i, md in enumerate(mesh_data_list):
            ...
            try:
                ...
                pred_result, t_infer = step_pred_inference(
                    ...
                    laplacian_config=pred_lap_config,
                )
                ...
                for tk, tk_cfg in zip(top_k_values, top_k_configs):
                    tk_L, tk_M, tk_asm_time = step_pred_reassemble(
                        pred_result['fwd_result'], patch_data, device,
                        laplacian_config=tk_cfg,
                    )
                    pfx = f'pred_tk{tk}'
